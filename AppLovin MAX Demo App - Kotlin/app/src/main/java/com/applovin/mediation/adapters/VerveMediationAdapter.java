    ...
    private class NativeAdListener implements HyBidNativeAdRequest.RequestListener, NativeAd.Listener {
        ...
        public void onRequestSuccess(NativeAd ad) {
            ...
            if (!TextUtils.isEmpty(nativeAd.getBannerUrl())) {
                PNBitmapDownloader bitmapDownloader = new PNBitmapDownloader();
                bitmapDownloader.download(nativeAd.getBannerUrl(), new PNBitmapDownloader.DownloadListener() {
                    @Override
                    public void onDownloadFinish(String url, Bitmap bannerBitmap) {
                        if (bannerBitmap != null) {
                            if (!TextUtils.isEmpty(nativeAd.getIconUrl())) {
                                bitmapDownloader.download(nativeAd.getIconUrl(), new PNBitmapDownloader.DownloadListener() {
                                    @Override
                                    public void onDownloadFinish(String url, Bitmap bitmap) {
                                        processNativeAd(bannerBitmap, bitmap);
                                    }

                                    @Override
                                    public void onDownloadFailed(String url, Exception exception) {
                                        processNativeAd(bannerBitmap, null);
                                    }
                                });
                            } else {
                                processNativeAd(bannerBitmap, null);
        ...
        private void processNativeAd(Bitmap bannerBitmap, Bitmap iconBitmap) {
            ...
            AppLovinSdkUtils.runOnUiThread(() -> {
                if (listener != null && nativeAd != null) {
                    ...
                    if (bannerBitmap != null) {
                        ...
                        bannerView.setImageBitmap(bannerBitmap);
                        builder.setMediaView(bannerView);
                    }

                    if (iconBitmap != null) {
                        MaxNativeAd.MaxNativeAdImage icon = new MaxNativeAd.MaxNativeAdImage(Uri.parse(nativeAd.getIconUrl()));
        ...
        private class MaxVerveNativeAd extends MaxNativeAd {
            private MaxVerveNativeAd(Builder builder) { super(builder); }
